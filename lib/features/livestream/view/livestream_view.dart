...
class _LivestreamViewState extends ConsumerState<LivestreamView> {
  ...
  Participant? _streamer;
  RemoteVideoTrack? _streamerVideoTrack;
  RemoteAudioTrack? _streamerAudioTrack;
  TrackPublication? _streamerPublication;
  ...
  void dispose() {
    (() async {
      ...
      _streamer?.removeListener(_onChange);
  ...
  void _onChange() {
    ...
    var visibleVideos = _streamer?.trackPublications.values.where((pub) {
  ...
  _subscribeMedia() async {
    for (RemoteParticipant remoteParticipant in _room.remoteParticipants.values) {
      if (remoteParticipant.attributes['isStreamer'] == "true") {
        for (RemoteTrackPublication<RemoteVideoTrack> remoteAudioTrackPublication in remoteParticipant.videoTrackPublications) {
          _streamerVideoTrack = remoteAudioTrackPublication.track;
        }

        for (RemoteTrackPublication<RemoteAudioTrack> remoteAudioTrackPublication in remoteParticipant.audioTrackPublications) {
          _streamerAudioTrack = remoteAudioTrackPublication.track;
        }
        _streamer = remoteParticipant;
        _streamer?.addListener(_onChange);
      }
    }
  }
