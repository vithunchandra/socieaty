...
import 'package:socieaty/features/livestream/view/live_join_failed_screen.dart';
...
class LivestreamWidget extends ConsumerStatefulWidget {
  final LiveRoom roomData;
  const LivestreamWidget(this.roomData, {super.key});

  @override
  ConsumerState<LivestreamWidget> createState() => _LivestreamViewState();
}

class _LivestreamViewState extends ConsumerState<LivestreamWidget> {
  late Room _room;
  ...
  @override
  void didUpdateWidget(covariant LivestreamWidget oldWidget) {
  ...
  _subscribeMedia() async {
    for (RemoteParticipant remoteParticipant in _room.remoteParticipants.values) {
      if (remoteParticipant.attributes['isStreamer'] == "true") {
        for (RemoteTrackPublication<RemoteVideoTrack> remoteVideoTrackPublication
  ...
  Widget build(BuildContext context) {
    if (_isDisconnected) {
      ...
      return const LiveJoinFailedScreen();
    }
                                            ...
                                            IconButton(
                                              ...
                                              onPressed: () {
                                                if (_commentController.text.isNotEmpty) {
                                                  if (_room.name != null &&
                                                      _room.name!.isNotEmpty) {
                                                    debugPrint(_room.name!);
                                                    ref
                                                        .read(livestreamViewModelProvider(
                                                                roomName: _room.name!)
                                                            .notifier)
                                                    ...
                                                    showSnackbar(
                                                        context, "Nama ruangan livestream kosong");
                                                  }
                                                }
                                              },
                                            ),
